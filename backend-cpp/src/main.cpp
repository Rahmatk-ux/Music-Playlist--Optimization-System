// main.cpp — Phase 3 Final
// Algorithms: dijkstra, tsp, mst, twoopt, kruskal, astar
#include <iostream>
#include <fstream>
#include <vector>
#include <algorithm>
...
RequestParams readRequestJSON(const string& filename) {
    ...
    if (!file.is_open()) { cerr << "Error: cannot open " << filename << endl; return params; }
    ...
    if (arrStart != string::npos) {
        ...
        if (bracketOpen != string::npos && bracketClose != string::npos) {
            string arr = content.substr(bracketOpen + 1, bracketClose - bracketOpen - 1);
            stringstream ss(arr);
            string item;
            while (getline(ss, item, ',')) {
                item.erase(remove_if(item.begin(), item.end(), ::isspace), item.end());
                if (!item.empty()) params.songIds.push_back(stoi(item));
            }
        }
    }

    // Helper: extract integer field value
...
// ── Run algorithm and export ──────────────────────────────────────────────────
void runAlgorithmAndExport(const RequestParams& params, const string& outputFile) {
    // Look up selected songs by ID
    vector<Song> selectedSongs;
    for (int id : params.songIds) {
        for (const Song& song : songDatabase) {
            if (song.id == id) { selectedSongs.push_back(song); break; }
        }
    }

    if (selectedSongs.empty()) { cerr << "Error: No songs selected!" << endl; return; }

    cout << "Running " << params.algorithm << " on " << selectedSongs.size() << " songs..." << endl;

    // Build graph
    Graph graph(selectedSongs);
    graph.buildGraph();

    // Find start/end vertex indices
    int startVertex = 0, endVertex = (int)selectedSongs.size() - 1;
    for (size_t i = 0; i < selectedSongs.size(); i++) {
        if (selectedSongs[i].id == params.startSongId) startVertex = i;
        if (selectedSongs[i].id == params.endSongId)   endVertex   = i;
    }
    ...
    } else if (params.algorithm == "twoopt") {
        TwoOpt twoopt(&graph);
        resultOrder = twoopt.run(startVertex);
        totalCost   = twoopt.getTourCost(resultOrder);
    ...
    } else if (params.algorithm == "kruskal") {
        Kruskal kruskal(&graph);
        vector<KEdge> edges = kruskal.runKruskal();
        resultOrder         = kruskal.mstToPlaylist(startVertex);
        totalCost           = kruskal.getTotalWeight(edges);

    } else if (params.algorithm == "astar") {
        AStar astar(&graph);
        resultOrder = astar.findPath(startVertex, endVertex);
        totalCost   = astar.getPathCost(resultOrder);

    } else {
        cerr << "Unknown algorithm: " << params.algorithm << endl;
        return;
    }

    // ── Write output.json ───────────────────────────────────────────────────
    ofstream file(outputFile);
    if (!file.is_open()) { cerr << "Error: cannot write " << outputFile << endl; return; }

    int    songCount = (int)selectedSongs.size();
    double avgCost   = songCount > 1 ? totalCost / (double)(resultOrder.size() - 1) : 0.0;

    file << "{" << endl;
    file << "  \"algorithm\":\"" << params.algorithm << "\"," << endl;
    file << "  \"songCount\":"   << songCount         << ","  << endl;
    file << "  \"totalCost\":"   << fixed << setprecision(1) << totalCost << "," << endl;
    file << "  \"avgCost\":"     << fixed << setprecision(1) << avgCost   << "," << endl;
    file << "  \"playlist\":["   << endl;
    ...
    cout << "Done. Total cost: " << fixed << setprecision(1) << totalCost << endl;
    cout << "Output written to " << outputFile << endl;
}

// ── main ──────────────────────────────────────────────────────────────────────
int main(int argc, char* argv[]) {
    cout << "\nMusic Playlist Optimizer (Phase 3) - " << songDatabase.size() << " songs\n" << endl;

    if (argc < 2) {
        cout << "Usage:" << endl;
        cout << "  " << argv[0] << " export [output.json]" << endl;
        cout << "  " << argv[0] << " run <input.json> [output.json]" << endl;
        cout << "\nAlgorithms: dijkstra | tsp | twoopt | mst | kruskal | astar" << endl;
        return 1;
    }
    ...
    if (command == "export") {
        ...
        if (argc < 3) { cerr << "Error: provide input JSON file" << endl; return 1; }
